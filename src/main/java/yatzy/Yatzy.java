...
public class Yatzy {
	...
	public static int ones(int[] values ) {		
     	return	Arrays.stream(values).filter(t -> t == 1 ).sum();		
	}
	

	public static int twos(int[] values) {
		return	Arrays.stream(values).filter(t -> t == 2 ).sum();
	}

	public static int threes(int[] values) {
		return	Arrays.stream(values).filter(t -> t == 3 ).sum();
	}

	public static int fours(int[] values) {
		return	Arrays.stream(values).filter(t -> t == 4 ).sum();
	}

	public static int fives(int[] values) {
		return	Arrays.stream(values).filter(t -> t == 5 ).sum();
	}

	public static int sixes(int[] values) {
		return	Arrays.stream(values).filter(t -> t == 6 ).sum();
	}

	public static int score_pair(int[] values) {
		Map<Integer, Integer> occurences = getOccurencesByValue(values);		
		Optional<Integer> score = occurences.entrySet().stream().filter(e -> e.getValue().intValue() == 2).map(t -> t.getKey()).max(Comparator.naturalOrder());
	...
	public static int four_of_a_kind(int[] values) {
		...
		long numberOfDiceWith4TimesSameNumber = occurences.entrySet().stream().filter(e -> e.getValue().intValue() >= 4).count();
	...
	public static int three_of_a_kind(int[] values) {
		int score = 0;
		Map<Integer, Integer> occurences = getOccurencesByValue(values);
		long numberOfDiceWith3AtLeastTimesSameNumber = occurences.entrySet().stream().filter(e -> e.getValue().intValue() >= 3).count();

		if (numberOfDiceWith3AtLeastTimesSameNumber >= 1) {
			score = occurences.entrySet().stream().filter(e -> e.getValue().intValue() >= 3).mapToInt(t -> t.getKey()).findFirst().getAsInt() * 3;
		}
	...
	public static int smallStraight(int[] values) {
		boolean allValuesDistinct = Arrays.stream(values).distinct().count() == values.length;
		int score = 0;
